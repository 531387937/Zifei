...
public class LevelTwoBoomerangController : MonoBehaviour
{
	...
	void OnTriggerEnter2D(Collider2D colliderObj)										//进入碰撞检测区域
	{
		if(m_bomerangState==1)
		{
			if(colliderObj.tag=="LevelTwoSpider")										//打中蜘蛛
			{
				...
				LevelTwoGameManager.Instance.SetSpiderBloodReduce(0.03f);				//蜘蛛血量减少
			}
		}
	}
	...
	void Update()
	{
		...
		{
		case 0:
			if(LevelTwoGameManager.Instance.GetBoomerangOut())							//如果发射了回旋镖
			{
				this.gameObject.transform.position = m_boomerangBirthPos.position;
				this.gameObject.GetComponent<SpriteRenderer>().enabled = true;
				m_heroScaleX = LevelTwoGameManager.Instance.GetHeroScaleX ();			//获取此时主角的朝向
	...
	void OnBoomerangMoveEnd()															//回旋镖已回归
	{
		...
		LevelTwoGameManager.Instance.SetBoomerangOut(false);							//回旋镖射出状态归零
