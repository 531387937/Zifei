...
public class LevelTwoHeroController : MonoBehaviour 
{
	...
	public delegate void heroHorizontalStateHandler(LevelTwoHeroController.heroHorizontalStates _newState);//定义委托和事件
	public static event heroHorizontalStateHandler onHorizontalStateChange; 
	
	public delegate void heroVerticalStateHandler(LevelTwoHeroController.heroVerticalStates _newState);//定义委托和事件
    ...
    private void Update()
    {
        if (LevelTwoGameManager.Instance.isPlatformType_Mobild) return;

        if (!LevelTwoGameManager.Instance.isUsualBtnCanClick) return;
	...
	void OnJoystickMove(MovingJoystick move)			//当摇杆处于移动状态，角色开始奔跑
	{
		...
		{
			m_joyStickDown = true;
            AudioManager.Instance.SoundPlay(Global.GetInstance().audioName_JoyStick);
	...
	void OnTriggerEnter2D(Collider2D colliderObj)					//进入碰撞检测区域
	{
		...
		case "PlatformBottom":										//最底层 不能下行区域
			m_downEnable = false;
			onVerticalStateChange(LevelTwoHeroController.heroVerticalStates.landing);
			LevelTwoHeroListener.m_invisibleColliderObj = colliderObj.gameObject;
			break;
	...
	void HeroInjuryBlinkCheck()
	{
		...
		{
			if(LevelTwoGameManager.Instance.GetHeroInjury())					//如果主角受伤
			{
		...
		else 																	//当前主角在闪烁
		{
			...
			{						
				...
				else if(m_heroBlinkCount==5)									//闪烁三次后
				{
					...
					LevelTwoGameManager.Instance.SetHeroInjury(false);			//受伤闪烁结束
				}
			}
		}
	}
	...
	void LateUpdate()					
	{
		HeroInjuryBlinkCheck ();
		//if(LevelTwoGameManager.Instance.GetJoyStickState())
		//{
		//	LevelTwoGameManager.Instance.SetJoyStickState(false);
		//	m_leftBtnCheck = false;
		//	m_rightBtnCheck = false;
		//	m_downBtnCheck = false;
		//	m_jumpBtnCheck = false;
		...
		if(this.GetComponent<Rigidbody2D>().velocity.y<-2f)													//如果主角具有向下的速度
		{
			if(onVerticalStateChange!=null)
				onVerticalStateChange(LevelTwoHeroController.heroVerticalStates.falling);		//下落状态
		}
		else
		{
			...
			{
				if(onVerticalStateChange!=null)
					onVerticalStateChange(LevelTwoHeroController.heroVerticalStates.jump);			//跳跃状态
			}
			else if(m_downBtnCheck)																//按下下行键
			{
				if(m_downEnable)
					if(onVerticalStateChange!=null)
						onVerticalStateChange(LevelTwoHeroController.heroVerticalStates.down);		//下行状态
			}
			else
			{
				if(onVerticalStateChange!=null)
					onVerticalStateChange(LevelTwoHeroController.heroVerticalStates.idle);			//空闲状态
			}
		}
