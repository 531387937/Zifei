...
public class LevelTwoMessageController : MonoBehaviour 
{
	...
	void Update()
	{
		...
		var messageType = LevelTwoGameManager.Instance.GetMessageType();

		if (messageType == 2)
		{
			m_messageContent = LevelTwoGameManager.Instance.GetMessageContent ();
			m_messageState = 1;
			m_coinGetState = 1;
			m_messageLabel[1].GetComponent<UILabel>().text = m_messageContent;
			m_allWords = false;
			LevelTwoGameManager.Instance.ClearMessage();
		}
		...
		if (isCanShowMessage) {
			...
			if (showDelayTimer >= showDelayTime) {
				...
				LevelTwoGameManager.Instance.isCanShowMsg = true;//
	...
	void CoinAni()
	{
		...
		case 3:
			...
			{	
				...
				//LevelTwoGameManager.Instance.SetMessageType(1, m_messageContent);
			}
			break;
		}
	}
