...
public class MessageController : MonoBehaviour 
{
    ...
    public float stopShowTime = 3f;//消息暂停显示的时间
    ...
    void Update()
	{
		...
		var messageType = GameManager.Instance.GetMessageType();

		if (messageType == 2)
		{
			m_messageContent = GameManager.Instance.GetMessageContent ();
			m_messageState = 1;
			m_coinGetState = 1;
		...
		if (isCanShowMessage) {
			...
			if (showDelayTimer >= showDelayTime) {
				...
				GameManager.Instance.isCanShowMsg = true;//
	...
	void CoinAni()
	{
		...
		case 2:
			...
			else if(m_coinTimer<0)																//上移结束
			{
				...
				m_coinAni.framesPerSecond = 7;												//闪光帧率
			}
			break;
		case 3:
			...
			{	
				...
				//GameManager.Instance.SetMessageType(1, m_messageContent);
	...
	void MessageAni()
	{
		...
		case 2:
			...
			{
				if(m_messageBox.GetComponent<UISprite>().spriteName=="ani_message03")		//播放到第3帧
				{
