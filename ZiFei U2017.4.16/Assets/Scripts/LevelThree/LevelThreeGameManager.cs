...
public class LevelThreeGameManager : MonoBehaviour 
{

    public GameObject deadPanelMainmenuBtnObj;
    public GameObject deadPanelObj;//死亡界面

    public static LevelThreeGameManager Instance;								//实例化
	...
	public GameObject m_levelThreeHero;											//主角
	...
	private int m_currentMoneyNum;												//当前主角的钱数
	private int m_currSaveIndex = 0;											//当前存至的档位编号



    private bool m_heroInjury = false;											//主角是否受伤
	private int m_messageType;													//消息类型
	private string m_messageContent;											//消息文字
	private int m_turnToCountry = 0;											//是否回到村庄(1开始界面 2非开始界面)
	private float m_movePanelSpeed;												//水平移动的平板的速度
	private bool[] m_heroOnMovePanel = new bool[2];								//主角站在水平移动的平板上
	private bool m_camTracker = true;											//相机是否跟随主角移动
	private bool m_boomerangOut = false;										//回旋镖是否发射出去
	private bool m_glassWaveEmit = false;										//眼镜光发出
	private int m_saveNPCNum = 8;												//主角遇到的NPC编号	
	private int m_getEndItem = 0;												//玩家是否过关
    ...
    void Awake()
	{
        ...
        m_levelThreeHero.SetActive(true);
    ...
    void Start()
	{
        ...
        m_currentMoneyNum = GameDataManager.Instance.gameData.GameCurrentData.heroMoney;       //获取主角金币数
        m_heroMoneyLabel.text = m_currentMoneyNum.ToString();
	...
	void Update()
	{
		...
		//转换scene的倒计时
		if (isNeedTurnScene) {
			turnSceneDelayTime -= Time.deltaTime;
			if (turnSceneDelayTime <= 0) {
	...
	void SceneNameInfoAni()														//场景名称提示条动画
	{
		...
		case 1:																	//第一阶段
			...
			m_sceneNameInfoLabel.text = "龙吼火山";								//当前场景名称
	...
	public void SetHeroBloodReduce(float _reduceNum)							//主角生命值减少量
	{
		...
		if(m_heroLifeObj.fillAmount<=0)											//如果主角生命值减完
			SetTurnToCountry(1);												//回村
	...
	{
		return m_levelThreeHero.transform.localScale.x;
	}
 ...
 //   public bool GetJoyStickState()												//摇杆是否可用变量
    ...
    //    if (m_barTaskState == 2)
    //        m_levelThreeHero.transform.GetChild(0).gameObject.SetActive(true);//开启道具装备
    //    else
    //        m_levelThreeHero.transform.GetChild(0).gameObject.SetActive(false);//隐藏道具装备
    //}
    ...
    public void SetTurnToCountry(int _turnCountry)                              //外部设定是否回村变量
    {
        ...
        if (_turnCountry == 1)                                                      //需要回村
        {
            ...
            m_levelThreeHero.SetActive(false);
        ...
        else if (_turnCountry == 2)//通关是的存档保存数据，并且切换场景
        {
            ...
            if (m_itemNum[18] == 1 && m_itemNum[14] == 0) //红色燃料的状态（0未得到 1得到）
                GameDataManager.Instance.gameData.GameCurrentData.getEndItemState = 3;                  //第3关成功标识
            else
                GameDataManager.Instance.gameData.GameCurrentData.getEndItemState = 0;					//
    ...
    public float GetMovePanelSpeed()											//外部获取水平移动的平板的速度
	{
		return m_movePanelSpeed;
	}
	public void SetMovePanelSpeed(float _speed)									//外部设置水平移动的平板的速度
	{
		m_movePanelSpeed = _speed;
	}

	public bool GetHeroOnMovePanel(int _index)									//外部获取主角是否站在移动的平板上
	{
		return m_heroOnMovePanel[_index];
	}
	public void SetHeroOnMovePanel(int _index, bool _onPanel)					//外部设置主角是否站在移动的平板上
	{
		m_heroOnMovePanel[_index] = _onPanel;
	}

	public bool GetCamTrackingValue()											//外部获取相机是否跟随主角移动
	{
		return m_camTracker;
	}
	public void SetCamTrackingValue(bool _tracking)								//外部设置相机是否跟随主角移动
	{
		m_camTracker = _tracking;
	}
	...
	public void SetEndItem(int _item)											//外部设置是否通关
	{
		...
		if(_item==2)
		{
			GameDataManager.Instance.gameData.GameCurrentData.seedState[2] = 1;					//获得第二关的通关物品
			GameDataManager.Instance.gameData.GameCurrentData.keyState[3] = 1;					//获得第二关的钥匙
		}
	}
	...
	public int SaveCurrData()																		//当前需要存档
	{
        ...
        _saveDataTemp.levelIndex = 6;                                                               //获取当前场景号(第六关
        _saveDataTemp.gameSceneName = SceneManager.GetActiveScene().name;//保存当前的scene名称

        _saveDataTemp.heroPosition = m_levelThreeHero.transform.position;                         //获取当前主角位置
        ...
        _saveDataTemp.NPCIndex = 10;                                                                 //获取当前遇到的最后NPC
	...
	{
		this.GetComponent<LevelThreeBackPackController>().enabled = false;							//关闭背包控制函数
        ...
        m_levelThreeHero.transform.position = _saveDataTemp.heroPosition;                         //指定当前主角位置
        m_currentMoneyNum = _saveDataTemp.heroMoney;                                           //指定当前钱数
        m_heroMoneyLabel.text = m_currentMoneyNum.ToString();                                      //指定右上主角状态栏钱数
        ...
        _saveDataTemp = null;																		//临时变量置空
		this.GetComponent<LevelThreeBackPackController>().enabled = true;							//激活背包控制函数
	}
