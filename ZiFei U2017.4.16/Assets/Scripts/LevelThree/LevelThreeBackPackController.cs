...
public class LevelThreeBackPackController : MonoBehaviour 
{
    ...
    void Awake()
    {
        ...
        UIEventListener.Get(m_mainMenuBtn.gameObject).onClick = OnMainMenuBtnClick;                 //点击背包按钮
    ...
    void InitTaskBox()                                                                                  //根据当前任务完成情况初始化任务框
    {
        ...
        {
            //print(LevelThreeGameManager.Instance.GetTaskIndexState(i) + "  <<< i: "+ i);
            if (LevelThreeGameManager.Instance.GetTaskIndexState(i) != 0)                                         //如果当前任务已接受
            {
    ...
    void OnMainMenuBtnClick(GameObject _mainBtn)                                   //点击了返回主界面
    {
        ...
        LevelThreeGameManager.Instance.CloseUsualBtn();
        CloseBackPack();

        LevelThreeGameManager.Instance.SetTurnToCountry(3);
    }
    ...
    void OnHelpBtnClick(GameObject _help)
    {
        ...
        //LevelThreeGameManager.Instance.SetHelpPanelOpen(true);
        //CloseBackPack();   
        //关闭背包
        ///TODO
    }
    ...
    void OnBpUseBtnClick(GameObject _useBtn)                                        //点击背包系统物品界面使用按钮
    {
        ...
        if (m_bpSelectItemIndex != 0 && m_selectMyItemNumMax != 0)                          //当前存在选中物体且选中物品数量不为0
        {
            ...
            {
                if (LevelThreeGameManager.Instance.GetItemEquipIndex() != 0)
                {
                    int _equipedIndex = LevelThreeGameManager.Instance.GetItemEquipIndex();
                ...
                LevelThreeGameManager.Instance.SetItemEquipIndex(m_bpSelectItemIndex - 4);            //指定当前装备的道具编号
            ...
            else if (m_bpSelectItemIndex == 3 || m_bpSelectItemIndex == 2)                                             //当前使用的是肉/jiu
            {
                int useCount = int.Parse(m_bpItemLabel[0].text);
                var newIndex = m_bpSelectItemIndex - 1;

                int _num = LevelThreeGameManager.Instance.GetItemNum(newIndex) - useCount;
                ...
                else
                {
                    ...
                    LevelThreeGameManager.Instance.SetItemNum(newIndex, _num);
                LevelThreeGameManager.Instance.SetMessageType(1, (useCount * 10 * newIndex) +" 点体力");
                LevelThreeGameManager.Instance.SetHeroBloodReduce(newIndex * - 0.1f* useCount);
    ...
    void OnBpDeleteBtnClick(GameObject go)
    {
        ...
        int residueNum = LevelThreeGameManager.Instance.GetItemNum(m_bpSelectItemIndex - 1) - _currNum;
        LevelThreeGameManager.Instance.SetItemNum(m_bpSelectItemIndex - 1, residueNum);								//更改已拥有的该物品数量
        ...
        else if (residueNum == 0)
        {
            ...
            if (m_bpSelectItemIndex > 4 && m_bpSelectItemIndex < 9)
            {
                ...
                LevelThreeGameManager.Instance.SetItemEquipIndex(0); // 取消Hero的饰品显示
            }
    ...
    void OnSaveBtnClick(GameObject _saveBtn)                                                            //点击了存档按钮
    {
        ...
        LevelThreeGameManager.Instance.CloseUsualBtn();                                                           //禁用常用按钮
    }
    ...
    void OnSaveInfoSureBtnClick(GameObject _okBtn)                                                        //点击了存档提示面板确定按钮
    {
        ...
        int _saveIndex = LevelThreeGameManager.Instance.SaveCurrData();                                           //存档
        _saveIndex++;
    ...
    void OnSaveInfoCancelBtnClick(GameObject go)
    {
        ...
        LevelThreeGameManager.Instance.OpenUsualBtn();															//禁用常用按钮
        ClearInfoPanelBtnEvent();
    }
    ...
    void OnSaveInfoOKBtnClick(GameObject go)
    {
        ...
        LevelThreeGameManager.Instance.OpenUsualBtn();                                                            //开启常用按钮
        ClearInfoPanelBtnEvent();
    ...
    void ShowSureReadDataInfoPanel(int index)
    {
        ...
        UIEventListener.Get(m_saveInfoSureBtn).onClick = SureSaveData;
    ...
    void SureSaveData(GameObject go)                                                                    //点击了档位按钮
    {
        AudioManager.Instance.SoundPlay(Global.GetInstance().audioName_BtnClick2);
        ...
        LevelThreeGameManager.Instance.GetSaveData(currentNeedReaddataIndex);                                                       //读取存档点1
        LevelThreeGameManager.Instance.SetTurnToCountry(3);
    ...
    void DeleteSaveData(int _index)                                                                 //点击了删档按钮											
    {
        ...
        LevelThreeGameManager.Instance.DeleteSaveData(_index);                                                    //存档点信息清空
    ...
    void OnBackPackBtnClick(GameObject _backPackBtn)                                                    //按下背包按钮 --lisong
    {
        ...
        m_bookMarkMoney.text = LevelThreeGameManager.Instance.GetCurrMoney().ToString();                          //指定当前拥有的钱数
        ...
        m_joyStick.SetActive(false);                                                     //关闭摇杆

        m_lifeObj.fillAmount = LevelThreeGameManager.Instance.GetBloodNum();                                      //外部获取主角血量值
        m_bpMusicSlider[0].value = LevelThreeGameManager.Instance.GetVolume(0);
        m_bpMusicSlider[1].value = LevelThreeGameManager.Instance.GetVolume(1);
    }
    ...
    void CloseBackPack()
    {
        ...
        LevelThreeGameManager.Instance.OpenUsualBtn();
        Time.timeScale = 1f;
    }
    ...
    void Update()
    {
        ...
        if (m_backPackOpening)                                                                          //如果背包打开
        {
            ...
            {
                case 1:                                                                                     //当前打开的为第一页
                    LevelThreeGameManager.Instance.SetVolume(0, m_bpMusicSlider[0].value);
                    LevelThreeGameManager.Instance.SetVolume(1, m_bpMusicSlider[1].value);
                    ...
                    if (Input.GetMouseButtonDown(0))                                                            //如果鼠标按下
                    {
                        ...
                        if (Physics.Raycast(ray, out hit))                                                  //检测鼠标点击区域
                        {
							...
							m_bpItemSelectBox.SetActive(true);											//显示选中框
                                ...
                                int itemCount = LevelThreeGameManager.Instance.GetItemNum(m_bpSelectItemIndex - 1);
                                ...
                                if (m_bpSelectItemIndex > 1 && m_bpSelectItemIndex < 9)
                                {
                                    ...
                                    { //点击的道具未装备
                                        int _isEquiped = LevelThreeGameManager.Instance.GetItemEquipIndex();
                ...
                case 4:                                                                                     //当前打开为任务界面
                    foreach (GameObject _taskBox in m_taskBoxList)                                                  //遍历所有任务
                    {
                        ...
                        if (LevelThreeGameManager.Instance.GetTaskIndexState(_index - 1) == 2)                                        //如果当前任务已完成
    ...
    void OnBackPackItemBtnClick(GameObject _backPackBtn)                                                    //物品界面
    {
        ...
        int _equipIndex = LevelThreeGameManager.Instance.GetItemEquipIndex();
        ...
        {
            if (LevelThreeGameManager.Instance.GetItemNum(i) != 0)                                                    //该物品数量不为0										
            {
    ...
    void OnBackPackTaskBtnClick(GameObject _backPackBtn)                                                    //任务页面
    {
        ...
        for (int i = m_taskBoxList.Count; i < 7; i++)                                                       //从已接受的任务向后遍历所有任务
        {
            ...
            if (LevelThreeGameManager.Instance.GetTaskIndexState(i) != 0)                                         //如果存新接受的任务
            {
                GameObject _taskBox = NGUITools.AddChild(m_taskBoxPanel, m_taskBox);                    //背包中新增任务栏
    ...
    void OnBackPackAchieveBtnClick(GameObject _backPackBtn)                                                // 成就页面
    {
        ...
        {
            if (LevelThreeGameManager.Instance.GetAchieveGot(i) == 1)                                                  //当前获得该成就								
            {
                count++;
        ...
        if (LevelThreeGameManager.Instance.GetTaskIndexState(2) >= 2)
        {
    ...
    void OnBackPackSaveDataBtnClick(GameObject _backPackBtn)                                                //存档界面
    {
        ...
        {
            if (LevelThreeGameManager.Instance.GetSaveBoxUsed(i))                                                 //如果该存档点已使用
            {
                m_bpSaveBox[i].GetComponent<BoxCollider>().enabled = true;                            //可以读档
                m_bpSaveDeleteBtn[i].SetActive(true);                       //可以删除存档
                m_saveTimeLabel[i].text = LevelThreeGameManager.Instance.GetSaveDataTime(i).ToString();           //显示当前存档时系统时间
                m_saveSceneNameLabel[i].text = LevelThreeGameManager.Instance.GetSaveSceneName(i);                                                //显示当前场景名称
                string _picName = "saveNPC" + LevelThreeGameManager.Instance.GetSaveNPCNum(i).ToString();
    ...
    void SetPageBtnActive(int _indexNew)                                                //设置按钮的激活形式
    {
        ...
        LevelThreeGameManager.Instance.SetBpIndex(m_openPageCurrIndex);							//翻页时指定当前背包打开的页数
        ...
        switch (m_openPageCurrIndex)                                                        //不同页数按钮激活状态
        {
            case 1:                                                                         //需开启第一页
                ...
                m_bookMarkMoney.depth = 10;
                m_bookMarkMoney.text = LevelThreeGameManager.Instance.GetCurrMoney().ToString();      //指定当前拥有的钱数
                m_lifeObj.fillAmount = LevelThreeGameManager.Instance.GetBloodNum();                  //指定当前血量情况跟
                m_bpMusicSlider[0].value = LevelThreeGameManager.Instance.GetVolume(0);               //指定当前音量情况
                m_bpMusicSlider[1].value = LevelThreeGameManager.Instance.GetVolume(1);
                for (int i = 0; i < 5; i++)
                    m_pageVirtualBtn[i].SetActive(false);
                for (int i = 5; i < 9; i++)
