...
public class LevelThreeMessageController : MonoBehaviour 
{
	...
	void Update()
	{
		...
		var messageType = LevelThreeGameManager.Instance.GetMessageType();

		if (messageType == 2)
		{
			m_messageContent = LevelThreeGameManager.Instance.GetMessageContent ();
			m_messageState = 1;
			m_coinGetState = 1;
			m_messageLabel[1].GetComponent<UILabel>().text = m_messageContent;
			m_allWords = false;
			LevelThreeGameManager.Instance.ClearMessage();
		}
		...
		if (isCanShowMessage) {
			...
			if (showDelayTimer >= showDelayTime) {
				...
				LevelThreeGameManager.Instance.isCanShowMsg = true;//
	...
	void CoinAni()
	{
		...
		case 3:
			...
			{	
				...
				//LevelThreeGameManager.Instance.SetMessageType(1, m_messageContent);
			}
			break;
		}
	}
