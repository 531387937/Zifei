...
public class LevelThreeBoomerang : MonoBehaviour 
{
	...
	void Update()
	{
		...
		{
		case 0:
			if(LevelThreeGameManager.Instance.GetBoomerangOut())							//如果发射了回旋镖
			{
				this.gameObject.transform.position = m_boomerangBirthPos.position;
				this.gameObject.GetComponent<SpriteRenderer>().enabled = true;
				m_heroScaleX = LevelThreeGameManager.Instance.GetHeroScaleX ();			//获取此时主角的朝向
	...
	void OnBoomerangMoveEnd()															//回旋镖已回归
	{
		...
		LevelThreeGameManager.Instance.SetBoomerangOut(false);							//回旋镖射出状态归零
