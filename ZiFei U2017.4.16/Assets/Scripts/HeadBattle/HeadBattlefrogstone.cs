...
public class HeadBattlefrogstone : MonoBehaviour 
{
	...
	void OnTriggerEnter2D(Collider2D colliderObj)						//进入碰撞检测区域
	{
		if(colliderObj.tag=="CountryHead")								//打中村长
		{
			...
			if(HeadBattleGameManager.Instance.GetAttackType()==0||HeadBattleGameManager.Instance.GetAttackType()==3)
				HeadBattleGameManager.Instance.SetHeadBloodReduce(0.03f);
			else
				HeadBattleGameManager.Instance.SetHeadBloodReduce(0.015f);
		}
