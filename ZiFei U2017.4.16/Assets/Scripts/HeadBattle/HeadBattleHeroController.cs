...
public class HeadBattleHeroController : MonoBehaviour 
{
	...
	public delegate void heroHorizontalStateHandler(HeadBattleHeroController.heroHorizontalStates _newState);//定义委托和事件
	public static event heroHorizontalStateHandler onHorizontalStateChange; 
	
	
	public delegate void heroVerticalStateHandler(HeadBattleHeroController.heroVerticalStates _newState);//定义委托和事件
    ...
    private void Update()
    {
        if (HeadBattleGameManager.Instance.isPlatformType_Mobild) return;

        if (!HeadBattleGameManager.Instance.isUsualBtnCanClick) return;
	...
	void OnTriggerEnter2D(Collider2D colliderObj)					//进入碰撞检测区域
	{
		...
		case "PlatformBottom":										//最底层 不能下行区域
			m_downEnable = false;
			onVerticalStateChange(HeadBattleHeroController.heroVerticalStates.landing);
			HeadBattleHeroListener.m_invisibleColliderObj = colliderObj.gameObject;
			break;
		}
	}
	...
	void HeroInjuryBlinkCheck()													//主角受伤闪烁
	{
		...
		{
			if(HeadBattleGameManager.Instance.GetHeroInjury())					//如果主角受伤
			{
		...
		else 																	//当前主角在闪烁
		{
			...
			{						
				...
				else if(m_heroBlinkCount==5)									//闪烁三次后
				{
					...
					HeadBattleGameManager.Instance.SetHeroInjury(false);		//受伤闪烁结束
				}
			}
		}
	}
	...
	void LateUpdate()					
	{
		...
		if(this.GetComponent<Rigidbody2D>().velocity.y<-2f)													//如果主角具有向下的速度
		{
			if(onVerticalStateChange!=null)
				onVerticalStateChange(HeadBattleHeroController.heroVerticalStates.falling);		//下落状态
		}
		else
		{
			...
			{
				if(onVerticalStateChange!=null)
					onVerticalStateChange(HeadBattleHeroController.heroVerticalStates.jump);			//跳跃状态
			}
			else if(m_downBtnCheck)																//按下下行键
			{
				if(m_downEnable)
					if(onVerticalStateChange!=null)
						onVerticalStateChange(HeadBattleHeroController.heroVerticalStates.down);		//下行状态
			}
			else
			{
				if(onVerticalStateChange!=null)
					onVerticalStateChange(HeadBattleHeroController.heroVerticalStates.idle);			//空闲状态
			}
		}
