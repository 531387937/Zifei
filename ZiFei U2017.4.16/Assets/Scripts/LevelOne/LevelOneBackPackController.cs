...
public class LevelOneBackPackController : MonoBehaviour
{
    ...
    public UILabel[] m_bpItemLabel;                                                                     //背包系统物品栏涉及文字（0数量 1名字 2介绍内容）
    ...
    private int m_openPageCurrIndex;                                                                    //当前打开的书页编号
    private bool m_backPackOpening = false;                                                             //背包是否打开
    ...
    private int currentNeedReaddataIndex;

    void Awake()
    {

        UIEventListener.Get(m_usualBtn[1].gameObject).onClick = OnBackPackBtnClick;                 //点击主界面背包按钮
        UIEventListener.Get(m_mainMenuBtn.gameObject).onClick = OnMainMenuBtnClick;                 //点击背包“主菜面"按钮
        //UIEventListener.Get(m_helpBtn.gameObject).onClick = OnHelpBtnClick;                         //点击背包了帮助按钮
        ...
        UIEventListener.Get(m_bpItemBtn[4].gameObject).onClick = OnBpDeleteBtnClick;                  //点击背包系统物品栏删除按钮 
        ...
        //隐藏存档按钮
        m_usualBtn[2].gameObject.SetActive(false);

        m_bpSaveBox[0].gameObject.GetComponent<UIButton>().enabled = false;
        m_bpSaveBox[1].gameObject.GetComponent<UIButton>().enabled = false;
        m_bpSaveBox[2].gameObject.GetComponent<UIButton>().enabled = false;

        m_helpBtn.SetActive(false);
    ...
    void InitTaskBox()                                                                                  //根据当前任务完成情况初始化任务框
    {
        ...
        {
            if (LevelOneGameManager.Instance.GetTaskIndexState(i) != 0)                                         //如果当前任务已接受
            {
    ...
    void OnMainMenuBtnClick(GameObject _mainBtn)                                   //点击了返回主界面
    {

        AudioManager.Instance.SoundPlay(Global.GetInstance().audioName_BtnClick2);
        LevelOneGameManager.Instance.CloseUsualBtn();
        CloseBackPack();

        LevelOneGameManager.Instance.SetTurnToCountry (3);//取消闯关
    ...
    void OnHelpBtnClick(GameObject _help)
    {
        ...
        //LevelOneGameManager.Instance.SetHelpPanelOpen(true);
        //CloseBackPack();   
        //关闭背包
        ///TODO
    }
    ...
    void OnBpUseBtnClick(GameObject _useBtn)                                        //点击背包系统物品界面使用按钮
    {
        ...
        if (m_bpSelectItemIndex != 0 && m_selectMyItemNumMax != 0)                          //当前存在选中物体且选中物品数量不为0
        {
            ...
            {
                if (LevelOneGameManager.Instance.GetItemEquipIndex() != 0)
                {
                    int _equipedIndex = LevelOneGameManager.Instance.GetItemEquipIndex();
                ...
                LevelOneGameManager.Instance.SetItemEquipIndex(m_bpSelectItemIndex-4);            //指定当前装备的道具编号
            ...
            else if (m_bpSelectItemIndex == 3 || m_bpSelectItemIndex == 2)                                              //当前使用的是肉
            {
                int useCount = int.Parse(m_bpItemLabel[0].text);
                var newIndex = m_bpSelectItemIndex - 1;

                int _num = LevelOneGameManager.Instance.GetItemNum(newIndex) - useCount;
                ...
                if (_num == 0)
                {
					...
					if(m_bpItemSelectBox != null) m_bpItemSelectBox.SetActive(false);                                                     //隐藏选中框
                ...
                LevelOneGameManager.Instance.SetItemNum(newIndex, _num);
                LevelOneGameManager.Instance.SetMessageType(1, (useCount * 10 * newIndex) + " 点体力");
                LevelOneGameManager.Instance.SetHeroBloodReduce(newIndex  * - 0.1f * useCount);
    ...
    void OnBpUnEquipBtnClick(GameObject _unEquipBtn)                                //点击背包系统物品界面解除按钮
    {
        ...
        if (m_bpSelectItemIndex != 0)                                                   //当前存在选中物体
        {
            ...
            {
                case 5:                                                                 //成人面具
                    //CheckBtnController.Instance.SetBarTaskState(3);                     //主角卸掉面具
    ...
    void OnBpDeleteBtnClick(GameObject go)
    {
        ...
        int residueNum = LevelOneGameManager.Instance.GetItemNum(m_bpSelectItemIndex - 1)  - _currNum;
        LevelOneGameManager.Instance.SetItemNum(m_bpSelectItemIndex - 1, residueNum);								//更改已拥有的该物品数量
        ...
        else if (residueNum == 0)
        {
            ...
            if (m_bpSelectItemIndex > 4 && m_bpSelectItemIndex < 9)
            {
                ...
                LevelOneGameManager.Instance.SetItemEquipIndex(0); // 取消Hero的饰品显示
            }
    ...
    void OnSaveBtnClick(GameObject _saveBtn)                                                            //点击了存档按钮
    {
        ...
        LevelOneGameManager.Instance.CloseUsualBtn();                                                           //禁用常用按钮
    }
    ...
    void OnSaveInfoSureBtnClick(GameObject _okBtn)                                                        //点击了存档提示面板确定按钮
    {
        ...
        int _saveIndex = LevelOneGameManager.Instance.SaveCurrData();                                           //存档
        _saveIndex++;
    ...
    void OnSaveInfoCancelBtnClick(GameObject go)
    {
        ...
        LevelOneGameManager.Instance.OpenUsualBtn();															//禁用常用按钮
        ClearInfoPanelBtnEvent();
    }
    ...
    void OnSaveInfoOKBtnClick(GameObject go)
    {
        ...
        LevelOneGameManager.Instance.OpenUsualBtn();                                                            //开启常用按钮
        ClearInfoPanelBtnEvent();

    }


    void ShowSureReadDataInfoPanel(int index)
    {
        UIEventListener.Get(m_saveInfoSureBtn).onClick = SureReadData;
        UIEventListener.Get(m_saveInfoCancelBtn).onClick = OnCancelOrOKDeleteDataBtnClick;

        m_saveInfoLabel.text = "确认读取档位  " + (index + 1).ToString()
                                + " 的数据存档吗？";//\n（[ff0000]所获将不会被保存[-]）";                  //提示信息文字
        m_saveInfoSureBtn.SetActive(true);
        m_saveInfoCancelBtn.SetActive(true);
        m_saveInfoOKBtn.SetActive(false);
        m_saveInfoPanel.SetActive(true); 
        currentNeedReaddataIndex = index;
    }


    void SureReadData(GameObject go)                                                                    //点击了档位按钮
    {
        AudioManager.Instance.SoundPlay(Global.GetInstance().audioName_BtnClick2);
        ...
        LevelOneGameManager.Instance.GetSaveData(currentNeedReaddataIndex);                                                       //读取存档点1
        LevelOneGameManager.Instance.SetTurnToCountry(3);
    ...
    void DeleteSaveData(int _index)                                                                 //点击了删档按钮											
    {
        ...
        LevelOneGameManager.Instance.DeleteSaveData(_index);                                                    //存档点信息清空
    ...
    void OnBackPackBtnClick(GameObject _backPackBtn)                                                    //按下背包按钮 --lisong
    {
        ...
        //背包打开 
        m_backPackOpening = true;   
        LevelOneGameManager.Instance.CloseUsualBtn();
        m_backPackPage[7].SetActive(true);                                                              //遮罩打开
        m_backPackPage[0].SetActive(true);
        //m_backPackPage[0].GetComponent<BoxCollider>().enabled = true;
        ...
        m_bookMarkMoney.text = LevelOneGameManager.Instance.GetCurrMoney().ToString();                          //指定当前拥有的钱数
        ...
        m_lifeObj.fillAmount = LevelOneGameManager.Instance.GetBloodNum();                                      //外部获取主角血量值
        m_bpMusicSlider [0].value = LevelOneGameManager.Instance.GetVolume (0);
        m_bpMusicSlider[1].value = LevelOneGameManager.Instance.GetVolume(1);
    }
    ...
    void CloseBackPack()
    {
        ...
        for (int i = 0; i < m_backPackPage.Length; i++)                                                                         //关闭所有背包图
            m_backPackPage[i].SetActive(false);
        for (int i = 0; i < m_pageVirtualBtn.Length-1; i++)                                                                         //禁用所有虚拟按钮
            m_pageVirtualBtn[i].SetActive(false);
        ...
        LevelOneGameManager.Instance.SetBpIndex(m_openPageCurrIndex);							//翻页时指定当前背包打开的页数

        LevelOneGameManager.Instance.OpenUsualBtn();
        Time.timeScale = 1f;
    }
    ...
    void Update()
    {
        ...
        if (m_backPackOpening)                                                                          //如果背包打开
        {
            ...
            if (Input.GetMouseButtonDown(0))                                                                //如果鼠标按下
            {
                ...
                if (Physics.Raycast(ray, out hit))                                                      //检测鼠标点击区域
                {
                    if (hit.collider.gameObject == m_backPackPage[7])                                   //如果点击了后边遮罩图 关闭背包
                    {
                        ...
                        m_joyStick.SetActive(true);                          //打开摇杆
                    }
                }
            }
            ...
            {
                case 1:                                                                                     //当前打开的为第一页
                    LevelOneGameManager.Instance.SetVolume(0, m_bpMusicSlider[0].value);
                    LevelOneGameManager.Instance.SetVolume(1, m_bpMusicSlider[1].value);
                ...
                case 2:                                                                                     //当前打开的为物品页面
                    if (Input.GetMouseButtonDown(0))                                                            //如果鼠标按下
                    {
                        ...
                        if (Physics.Raycast(ray, out hit))                                                  //检测鼠标点击区域
                        {
							...
							m_bpItemSelectBox.SetActive(true);		

                                m_bpSelectItemIndex = int.Parse(hit.collider.gameObject.name);              //通过名字获取点击的物品索引

                                int itemCount = LevelOneGameManager.Instance.GetItemNum(m_bpSelectItemIndex - 1);
                                ...
                                if (m_bpSelectItemIndex > 1 && m_bpSelectItemIndex < 9)
                                {
                                    ...
                                    if (m_bpSelectItemIndex > 4)
                                    { //点击的道具未装备
                                        int _isEquiped = LevelOneGameManager.Instance.GetItemEquipIndex();
                ...
                case 4:                                                                                     //当前打开为任务界面
                    foreach (GameObject _taskBox in m_taskBoxList)                                                  //遍历所有任务
                    {
                        ...
                        if (LevelOneGameManager.Instance.GetTaskIndexState(_index - 1) == 2)                                        //如果当前任务已完成
    ...
    void OnBackPackItemBtnClick(GameObject _backPackBtn)                                                    //物品界面
    {
        ...
        int _equipIndex = LevelOneGameManager.Instance.GetItemEquipIndex();
        ...
        {
            if (LevelOneGameManager.Instance.GetItemNum(i) != 0)                                                    //该物品数量不为0										
            {
    ...
    void OnBackPackTaskBtnClick(GameObject _backPackBtn)                                                    //任务页面
    {
        ...
        for (int i = m_taskBoxList.Count; i < 7; i++)                                                       //从已接受的任务向后遍历所有任务
        {
            ...
            if (LevelOneGameManager.Instance.GetTaskIndexState(i) != 0)                                         //如果存新接受的任务
            {
                GameObject _taskBox = NGUITools.AddChild(m_taskBoxPanel, m_taskBox);                    //背包中新增任务栏
    ...
    void OnBackPackAchieveBtnClick(GameObject _backPackBtn)                                                // 成就页面
    {
        ...
        {
            if (LevelOneGameManager.Instance.GetAchieveGot(i) == 1)                                              //当前获得该成就								
            {
                count++;
        ...
        if (LevelOneGameManager.Instance.GetTaskIndexState(2) >= 2)
        {
    ...
    void OnBackPackSaveDataBtnClick(GameObject _backPackBtn)                                                //存档界面
    {
        ...
        {
            if (LevelOneGameManager.Instance.GetSaveBoxUsed(i))                                                 //如果该存档点已使用
            {
                m_bpSaveBox[i].GetComponent<BoxCollider>().enabled = true;                            //可以读档
                m_bpSaveDeleteBtn[i].SetActive(true);                       //可以删除存档
                m_saveTimeLabel[i].text = LevelOneGameManager.Instance.GetSaveDataTime(i).ToString();           //显示当前存档时系统时间
                m_saveSceneNameLabel[i].text = LevelOneGameManager.Instance.GetSaveSceneName(i);                //显示当前场景名称
                string _picName = "saveNPC" + LevelOneGameManager.Instance.GetSaveNPCNum(i).ToString();
    ...
    void SetPageBtnActive(int _indexNew)                                                //设置按钮的激活形式
    {
        ...
        switch (m_openPageCurrIndex)                                                        //不同页数按钮激活状态
        {
            case 1:                                                                         //需开启第一页
                ...
                m_bookMarkMoney.depth = 10;
                m_bookMarkMoney.text = LevelOneGameManager.Instance.GetCurrMoney().ToString();      //指定当前拥有的钱数
                m_lifeObj.fillAmount = LevelOneGameManager.Instance.GetBloodNum();                  //指定当前血量情况跟
                m_bpMusicSlider[0].value = LevelOneGameManager.Instance.GetVolume(0);               //指定当前音量情况
                m_bpMusicSlider[1].value = LevelOneGameManager.Instance.GetVolume(1);
                for (int i = 0; i < 5; i++)
                    m_pageVirtualBtn[i].SetActive(false);
                for (int i = 5; i < 9; i++)
