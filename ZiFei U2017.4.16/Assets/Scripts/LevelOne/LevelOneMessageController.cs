...
public class LevelOneMessageController : MonoBehaviour 
{
	...
	void Update()
	{
		...
		var messageType = LevelOneGameManager.Instance.GetMessageType();

		if (messageType == 2)
		{
			m_messageContent = LevelOneGameManager.Instance.GetMessageContent ();
			m_messageState = 1;
			m_coinGetState = 1;
			m_messageLabel[1].GetComponent<UILabel>().text = m_messageContent;
			m_allWords = false;
			LevelOneGameManager.Instance.ClearMessage();
		}
		...
		if (isCanShowMessage) {
			...
			if (showDelayTimer >= showDelayTime) {
				...
				LevelOneGameManager.Instance.isCanShowMsg = true;//
	...
	void CoinAni()
	{
		...
		case 3:
			...
			{	
				...
				//LevelOneGameManager.Instance.SetMessageType(1, m_messageContent);
			}
			break;
		}
	}
